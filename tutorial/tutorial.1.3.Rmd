...
*We would like to point out that when we speak of 'predictions', we use the term in its [statistical sense](https://en.wikipedia.org/wiki/Prediction#Statistics). We do not mean ’forecasting, as in forecasting future influenza cases. We are 'Nowcasting': We observe what people were doing on the internet until today and infer from this information how many influenza cases there are about now.*
...
# Identifying 25 related search queries
...
The loops to download the search queries are rather messy. We will just load them from github, where you can have a closer [look at them](https://raw.githubusercontent.com/projectflutrend/pft.2/master/quickfunctions/pft_ask_google), if you want. 
...
Therefore, one should use time series cross validation. This means, we will train a model using the data from the first data point until week x and validate the results using data from the next k-weeks. In each validation round, we will move 1 week forward, while the origin stays the same. The amount of data the model is being trained on increases with every round (A model is trained using data from 2010-W30 to 2011-W30 and validated with data from 2011-W31 and 2012-W30, in the next round, the model is trained from 2010-W30 to 2011-W31 and validated with data from 2011-W32 and W31, and so on). You could also consider using a moving origin (i.e. the amount of training data stays the same), if you think online behaviour from six years ago is not relevant any more and models should only be based on data from the last two seasons, for example. k basically represents the time period in which the model is being updated - If k = 52, a model would remain the same for a whole year without being updated with actual incidence data. We chose a k of 4, meaning a model will be evaluated using 4 weeks of forward rolling test data.
  
  
  __MISUNDERSTOOD HORIZON, NEED TO RUN ALL MODELS AGAIN...!!! ARGHS!__
```{r}
...
__Cave:__ Running the code below can take several hours! 
...
# lasso regression (glmnet)
  # lasso grid
  lassoGrid <- expand.grid(.alpha = c(.2, .4, .6, .8),.lambda = seq(.05, 1.5, length = 50)) # refined grid
  # Model
...
We have stored the results of the previous analyses on github:

```{r load models from github, message=FALSE, warning=FALSE}
...
##### CV RMSE results over all tuning paramters
...
eval(parse(text = rmse.boxplots.function))

rmse.plots(models.de$result.list,box=1) +
  ylim(1,3.5)

rmse.plots(models.de$result.list,box=0,parallel = 1) 
```
*"Each line represents the results for a common CV holdout set"*
<br>



__Showing lowest CV RMSE results per model__

```{r  lowest CV RMSE per model}
# lowest CV RMSE per model
rmse.means = rmse.plots(models.de$result.list,box=0,mins=1)
mins = by(rmse.means$value,rmse.means$model,function(x){min(x,na.rm=T)})
plot(1:length(mins),mins[order(mins)],type="h",col=1:12,ylab="CV RMSE",xlab="model",main="Best CV RMSE result",ylim=c(0,3),xlim=c(0,13))
text(1:length(mins),mins[order(mins)],paste(names(mins[order(mins)]),"=",round(mins[order(mins)],3)), cex=0.6, pos=3,col=1:12,srt=45)
...
We can also evaluate each model individually, to see....
...
```{r plotting individual model results, fig.width=7,fig.height=21}

grid.arrange(plot(models.de$result.list[[1]],
                  main=paste("CV RMSE =",round(models.de$eval.list[[1]]$lowest.rmse,4))),
             models.de$eval.list[[1]]$plots$pred.plot,
             plot(models.de$result.list[[2]] ,
                  main=paste("CV RMSE =",round(models.de$eval.list[[2]]$lowest.rmse,4))),
             ...
             plot(models.de$result.list[[4]] ,
                  main=paste("CV RMSE =",round(models.de$eval.list[[4]]$lowest.rmse,4))),
             models.de$eval.list[[4]]$plots$pred.plot,
             plot(models.de$result.list[[5]] ,
                  main=paste("CV RMSE =",round(models.de$eval.list[[5]]$lowest.rmse,4))),
             models.de$eval.list[[5]]$plots$pred.plot,
             plot(models.de$result.list[[6]] ,
                  main=paste("CV RMSE =",round(models.de$eval.list[[6]]$lowest.rmse,4))),
             models.de$eval.list[[6]]$plots$pred.plot,
             plot(models.de$result.list[[7]] ,
                  main=paste("CV RMSE =",round(models.de$eval.list[[7]]$lowest.rmse,4))),
             models.de$eval.list[[7]]$plots$pred.plot,
             ggplot() + ggtitle(
                 label= paste("CV RMSE =",round(models.de$eval.list[[8]]$lowest.rmse,4))),
             models.de$eval.list[[8]]$plots$pred.plot,
             plot(models.de$result.list[[9]] ,
                  main=paste("CV RMSE =",round(models.de$eval.list[[9]]$lowest.rmse,4))),
             models.de$eval.list[[9]]$plots$pred.plot,
             plot(models.de$result.list[[10]] ,
                  main=paste("CV RMSE =",round(models.de$eval.list[[10]]$lowest.rmse,4))),
             models.de$eval.list[[10]]$plots$pred.plot,
             plot(models.de$result.list[[11]] ,
                  main=paste("CV RMSE =",round(models.de$eval.list[[11]]$lowest.rmse,4))),
             models.de$eval.list[[11]]$plots$pred.plot,
             plot(models.de$result.list[[12]] ,
                  main=paste("CV RMSE =",round(models.de$eval.list[[12]]$lowest.rmse,4))),
             models.de$eval.list[[12]]$plots$pred.plot,
             top="Comparing Models",
             widths=2:3, ncol=2)
...
#### Model selection
The top models don't differ much.

All models underpredicted the incidence withn the test data, which was unusually high. ...
...
Back to our null-model: Did the prediction model performed better than the forecast?



#### Influenza-Nowcasting
Now, we are going to 'Nowcast' the current influenza incidence in Germany.
